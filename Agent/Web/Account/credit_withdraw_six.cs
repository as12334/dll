namespace Agent.Web.Account
{
    ...
    public class credit_withdraw_six : MemberPageBase
    {
            ...
            if (str3.Equals("rhrg"))
            {
                ...
                if (!Utils.IsDouble(s))
                {
                    if (this.ispc.Equals("1"))
                    {
                        ...
                        base.Response.Write(base.ShowDialogBox("(⑥)提現格式不正確！", "", 400));
                        base.Response.End();
                    }
                    else
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(string.Format("<script>alert('{0}');</script>", "(⑥)提現格式不正確！"));
                        base.Response.End();
                    }
                }
                if (double.Parse(s) < 1.0)
                {
                    if (this.ispc.Equals("1"))
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(base.ShowDialogBox("(⑥)提現金額不能小於1！", "", 400));
                        base.Response.End();
                    }
                    else
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(string.Format("<script>alert('{0}');</script>", "(⑥)提現金額不能小於1！"));
                        base.Response.End();
                    }
                }
                if (double.Parse(s) > double.Parse(this.edit_model.get_six_usable_credit().ToString()))
                {
                    if (this.ispc.Equals("1"))
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(base.ShowDialogBox(string.Format("(⑥)提現金額不能大於{0}！", Utils.GetMathRound(double.Parse(this.edit_model.get_six_usable_credit().ToString()), null)), "", 400));
                        base.Response.End();
                    }
                    else
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(string.Format("<script>alert('(⑥)提現金額不能大於{0}');</script>", Utils.GetMathRound(double.Parse(this.edit_model.get_six_usable_credit().ToString()), null)));
                        base.Response.End();
                    }
                }
                if (CallBLL.cz_users_bll.UserWithdraw_six(this.edit_model, this.up_edit_model, double.Parse(s), int.Parse(this.mlid), this.userModel.get_u_name(), (this.userModel.get_users_child_session() != null) ? this.userModel.get_users_child_session().get_u_name() : "", 0) > 0)
                {
                    if (this.ispc.Equals("1"))
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(base.ShowDialogBox("(⑥)提現成功！", base.UserReturnBackUrl, 3));
                        base.Response.End();
                    }
                    else
                    {
                        base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                        this.Hy_DeleteCreditLock();
                        base.Response.Write(string.Format("<script>alert('{0}');location.href='{1}';</script>", "(⑥)提現成功！", base.UserReturnBackUrl));
                        base.Response.End();
                    }
                }
                else if (this.ispc.Equals("1"))
                {
                    base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                    this.Hy_DeleteCreditLock();
                    base.Response.Write(base.ShowDialogBox("(⑥)提現失敗！", "", 400));
                    base.Response.End();
                }
                else
                {
                    base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                    this.Hy_DeleteCreditLock();
                    base.Response.Write(string.Format("<script>alert('{0}');</script>", "(⑥)提現失敗！"));
                    base.Response.End();
                }
            }
        }
