namespace Agent.Web.m.Account
{
    ...
    public class zd_add : MemberPageBase_Mobile
    {
        ...
        private void AddUser()
        {
            ...
            {
                base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                base.noRightOptMsg("⑥合彩: 股東占成 不可高於" + this.six_rate_d + "，請修改！");
            }
            if (str6.Equals("1"))
            {
            ...
            {
                base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                base.noRightOptMsg("快彩: 股東占成 不可高於" + this.kc_rate_d + "，請修改！");
            }
            if (str11.Equals("1"))
            {
            ...
            if (CallBLL.cz_users_bll.AddUserInfo(model, _kc2, _six, ref str19))
            {
                ...
                base.successOptMsg("添加總代理成功！", data);
            }
            else
            {
                base.Un_User_Lock(rateKCByUserName.get_fgs_name());
                base.noRightOptMsg("添加總代理失敗！");
            }
        }
        ...
        private void GetRateInfo(string g_uid)
        {
            ...
            {
                Dictionary<string, string> rateByUID = CallBLL.cz_rate_six_bll.GetRateByUID(g_uid, "gd");
                if (rateByUID != null)
                {
                ...
                Dictionary<string, string> dictionary2 = CallBLL.cz_rate_kc_bll.GetRateByUID(g_uid, "gd");
                if (dictionary2 != null)
                {
        ...
        private void InitSelect()
        {
            ...
            if (string.IsNullOrEmpty(this.upuid))
            {
                ...
                str = "select u_name from  cz_rate_six  where u_type='gd' " + str2;
            }
            else
            {
                cz_users userInfoByUID = CallBLL.cz_users_bll.GetUserInfoByUID(this.upuid);
                str = string.Format("select u_name from cz_rate_six where u_type='gd' and {0}_name='{1}'", userInfoByUID.get_u_type(), userInfoByUID.get_u_name());
            }
            DataTable userListByAddUser = CallBLL.cz_users_bll.GetUserListByAddUser(str, "gd");
